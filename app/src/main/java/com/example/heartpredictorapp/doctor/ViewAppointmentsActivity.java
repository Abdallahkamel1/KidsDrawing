...
import com.example.heartpredictorapp.adapters.ManageAppointmentAdapter;
import com.example.heartpredictorapp.adapters.ViewAppointmentAdapter;
...
public class ViewAppointmentsActivity extends AppCompatActivity {
    ...
    ArrayList<Appointment> appointArrayList;
    ViewAppointmentAdapter appointAdapter;
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for(DataSnapshot dataSnapshot : snapshot.getChildren() ){
                    ...
                    try {
                        Date appointmentDate = dateFormat.parse(appoint.getAppDate());
                        if (appoint.getDoctorID().equals(LoginActivity.doctor.getDoctorID()) && appointmentDate.after(currentDate)) {
                            appointArrayList.add(appoint); // Only add upcoming appointments
                        }
                    } catch (ParseException e) {
                        e.printStackTrace(); // Handle incorrect date format
                    }
                }
                appointAdapter = new ViewAppointmentAdapter(getApplicationContext(), appointArrayList);
                recyclerView.setAdapter(appointAdapter);
